...
function logipro_ozy_vp_get_custom_fonts_kirki() {
	$ozy_fonts = get_posts(array(
		'posts_per_page' 	=> -1,
		'post_type' 		=> 'ozy_fonts'
	));
	
	$fonts = array(); $weight_arr = array(); $style_arr = array(); $font_file_arr = array();
	foreach ($ozy_fonts as $post) {
		$font_grp = vp_metabox('ozy_logipro_meta_font.ozy_logipro_meta_font_group', null, $post->ID);
		if(is_array($font_grp) && count($font_grp) > 0) {
			foreach($font_grp as $fnt) {
				$weight = $fnt['ozy_logipro_meta_font_weight'];
				if(!in_array($weight, $weight_arr)) array_push($weight_arr, $weight);
				
				$style = $fnt['ozy_logipro_meta_font_style'];
				if(!in_array($style, $style_arr)) array_push($style_arr, $style);
				
				$font_file = $fnt['ozy_logipro_meta_font_ttf'];
				if(!in_array($font_file, $font_file_arr)) array_push($font_file_arr, array($weight, $font_file));
			}
		}
		
		$fonts[] = array(
			"kind" 			=> "webfonts#webfont", 
			"family" 		=> '___' . $post->post_title, 
			"category" 		=> "serif", 
			"variants" 		=> $weight_arr, //style might be added to like, 500italic
			"subsets" 		=> array("latin"), 
			"version" 		=> "v1", 
			"lastModified" 	=> date("Y-m-d", get_the_time('U', $post->ID)),
			"files"			=> $font_file_arr
		);
	}
...
function logipro_ozy_vp_get_custom_fonts($fonts = null) {
	$ozy_fonts = get_posts(array(
		'posts_per_page' 	=> -1,
		'post_type' 		=> 'ozy_fonts'
	));
...
function vp_get_gwf_family(){
	...
	$fonts = logipro_ozy_vp_get_custom_fonts(json_decode($fonts));
...
function vp_get_social_medias_single_arr() {
	$socmeds = array(
		'blogger' => 'Blogger',
		'behance' => 'Behance',
		'delicious' => 'Delicious',
