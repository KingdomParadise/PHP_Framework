...
class VP_Metabox extends WPAlchemy_MetaBox
{
	...
	function __construct($arr)
	{
		...
		if( $this->is_dev_mode )
		{
			$this->title = esc_attr__('[Development Mode] ', 'logipro') . $this->title;
		}
	...
	function _enview($fields)
	{
		foreach ($fields as $name => $field)
		{
			...
			{
				logipro_e_html($this->_render_repeating_group($field));
			}
			else if( is_array($field) and !$field['repeating'] )
			{
				logipro_e_html($this->_render_group($field));
			}
			else
			{
				logipro_e_html($this->_render_field($field));
			}
		}
	}
	...
	function _render_repeating_group($group)
	{
		...
		foreach ($group['groups'] as $g)
		{
			...
			$html .= '<div class="vp-wpa-group-heading"><a href="#" class="vp-wpa-group-title">' . $icon . $group['title'] . '</a><a href="#" class="dodelete vp-wpa-group-remove" title="'. esc_attr__('Remove', 'logipro') .'"><i class="fa fa-times"></i> '. esc_attr__('Remove', 'logipro') .'</a></div>';
			$html .= '<div class="vp-controls' . ((!$is_first) ? ' vp-hide' : '') . '">';
		...
		$html .= '<a href="#" class="button button-large docopy-' . esc_attr($name) . '">'. esc_attr__('Add More', 'logipro') . ' ' . esc_html($group['title']) . '</a>';
		$html .= '</div>';
