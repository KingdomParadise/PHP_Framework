jQuery(document).ready(function($) {
	jQuery(document).on( 'click', '.ozy-purchase-code-multi-use-notice .purchase-code-multi-use-notice-dismiss-link', function(e) {
		e.preventDefault();
		jQuery('.ozy-purchase-code-multi-use-notice .notice-dismiss').click();
		return false;
	});
	jQuery(document).on( 'click', '.ozy-purchase-code-multi-use-notice .notice-dismiss', function() {
		jQuery.ajax({
			url: ajaxurl,
			data: {
				action: 'logipro_purchase_code_multi_use_notice_dismiss'
			}
		});
	});
	...
	$('#ozy_import_revo_slider').click(function(){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: { action: 'logipro_ozy_ajax_auto_install_revo_slider', selected_sliders: selected_sliders_data},
			cache: false,
	...
	$('div[id*="ozy_logipro_meta_post"]').css('overflow', 'inherit');

	if (typeof ozyAdminParams != 'undefined') {
		jQuery.ajax({
	...
	*/
	...
	$(document).on('click', '.edit-menu-item-edit-style', function() {
		...
		//load settings
		var get_params = ozy_current_target_menu_style_box.siblings('textarea').val() ? jQuery.parseJSON( ozy_current_target_menu_style_box.siblings('textarea').val() ) : null;;

		//set loaded values
		if (get_params !== undefined && get_params !== null) {
			if(get_params.use_custom !== undefined) { $('#custom-menu-use-custom').prop('checked', (get_params.use_custom === '1' ? true : false)); }
			...
			if(get_params.request_rate !== undefined) { $('#custom-menu-request-a-rate-form').prop('checked', (get_params.request_rate === '1' ? true : false)); }
			if(get_params.tracking_form !== undefined) { $('#custom-menu-tracking').prop('checked', (get_params.tracking_form === '1' ? true : false)); }

		}else{
			$('#custom-menu-use-custom').prop('checked', false);
			$('#custom-menu-fn-color').val('#ffffff').minicolors('destroy').minicolors();
			$('#custom-menu-bg-color').val('#352c66').minicolors('destroy').minicolors();
			$('#custom-menu-request-a-rate-form').prop('checked', false);
			$('#custom-menu-tracking').prop('checked', false);
	...
	$(document).on('click', '#custom-menu-bg-apply', function(){
		if(ozy_current_target_menu_style_box != null) {
			var pass_params =  '{';
			pass_params += '"use_custom":"'+ ($('#custom-menu-use-custom').is(':checked') ? '1' : '0') +'",';
			pass_params += '"bg_color":"'+ $('#custom-menu-bg-color').val() +'",';
			pass_params += '"fn_color":"'+ $('#custom-menu-fn-color').val() +'",';

			pass_params += '"request_rate":"'+ ($('#custom-menu-request-a-rate-form').is(':checked') ? '1' : '0') +'",';
			pass_params += '"tracking_form":"'+ ($('#custom-menu-tracking').is(':checked') ? '1' : '0') +'"';
