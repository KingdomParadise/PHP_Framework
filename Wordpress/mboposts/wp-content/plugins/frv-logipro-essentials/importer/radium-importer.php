 ...
 if ( !class_exists( 'Radium_Theme_Importer' ) ) {
	...
	class Radium_Theme_Importer {
	    ...
	    public function __construct() {
			...
			/* Demo Type Selection */
			if(isset($_POST['demo-data-type'])) {		
				$demo_data_type_selection = sanitize_text_field($_POST['demo-data-type']);
				if($demo_data_type_selection && $demo_data_type_selection != NULL && $demo_data_type_selection != '') {
					if($demo_data_type_selection === 'all') {
						$this->widgets_file_name 		= $demo_data_type_selection . '-' . $this->widgets_file_name;
						$this->content_demo_file_name 	= $demo_data_type_selection . '-' . $this->content_demo_file_name;
					}
				}
			}
    	...
    	public function intro_html() {
			$memory_limit = logipro_ozy_return_bytes(ini_get('memory_limit'));
			...
			echo '	<tr><td>Memmory Limit</td><td class='. logipro_ozy_check_limit_size($memory_limit, 67108864) . '>'. ($memory_limit / 1024 / 1024) .'M</td><td>64M</td></tr>';
			echo '	<tr><td>Max Execution Time</td><td class='. logipro_ozy_check_limit_size($max_execution_time, 180) . '>'. $max_execution_time .'s</td><td>180s</td></tr>';
			echo '	<tr><td>Post Max Size</td><td class='. logipro_ozy_check_limit_size($post_max_size, 32) . '>'. $post_max_size .'</td><td>32M</td></tr>';
			echo '	<tr><td>Upload Max Size</td><td class='. logipro_ozy_check_limit_size($upload_max_filesize, 32) . '>'. $upload_max_filesize .'</td><td>32M</td></tr>';		
			echo '	</tbody>';
			echo '</table>';
	    ...
	    public function demo_installer() {
			...
			<?php
			/* Check if is WordPress Importer plugin is active */
			if(function_exists('wordpress_importer_init')) {
					echo '<h3 style="font-size:2rem;" class="red-text blink">WordPress Importer plugin is active. Please deactivate the plugin.</h3>';
			}
			?>			
			...
			<div class="radium-importer-wrap" data-demo-id="1"  data-nonce="<?php echo wp_create_nonce('radium-demo-code'); ?>">
		        ...
		        <form method="post">
					...
					<div class="generic-info-box orange-box">
						<label for="demo-data-type">Please select: </label>
						<select name="demo-data-type" id="demo-data-type">
							<option value="all" selected>Import Everything</option>
						</select>
					</div>
			...
			echo '<p>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="3d_clay_render_slider" checked>3D Clay Render Slider</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="carousel-gallery" checked>Carousel Gallery</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="greyscale" checked>Greyscale</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="mask-showcase" checked>Mask Showcase</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="overexposure-transition" checked>Overexposure Transition</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="services" checked>Services</label><br>';
			echo '</p>';
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			if ( ! $fail ) {
	    				...
	    				if ( $sidebar_available ) {
	    					...
	    					$widget_message = esc_attr__( 'Imported', 'radium' );
	    				} else {
	    					$widget_message_type = 'warning';
	    					$widget_message = esc_attr__( 'Imported to Inactive', 'radium' );
	    				}
	    			...
	    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_attr__( 'No Title', 'radium' ); // show "No Title" if widget instance is untitled
